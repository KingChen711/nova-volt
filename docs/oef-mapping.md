...
| **Edge connector** | 8. Equipment Integration | Gateway MQTT/Sparkplug B đứng ở `dmz-net`, store-and-forward xuống đĩa | `src/Workers/Nvm.EdgeGateway/` | `đang làm` | Subscribe EMQX, decode Sparkplug B, đóng dấu `gateway_timestamp`, đệm append-only có CRC + cursor fsync (`ADR-028`). Rate limit + `Retry-After` khi xả (`ADR-029`). Theo dõi `NBIRTH`/`NDEATH`/`bdSeq`, `seq` nhảy cóc thì xin **rebirth**. Lab D3: tắt backend 2 phút → **6.272 message trên đĩa, 0 restart, 0 row lệch**; backlog tiêu **185 s** (ngân sách 180 s — **trượt 5 giây**). Chưa `xong` vì đúng cái ngân sách đó chưa đạt |
| **Data Collection** | 8. Equipment Integration | Ingestion nhận batch protobuf qua HTTP, ghi TimescaleDB, và nhận cả CSV file drop | `src/Workers/Nvm.Ingestion/` | `đang làm` | Điểm bắc cầu duy nhất `dmz-net` → `it-net` (K11, K12). Hai adapter — MQTT và file drop — đi **cùng một cửa dedup**, cùng một natural key. `clock_quality` phân loại `Good`/`Drifted`/`Unknown` và **vẫn ghi** khi lệch (N15). D2: **p95 lag < 5 s ở 5.000 msg/s**. Chưa `xong` vì hypertable, compression và retention là **M3** |
