...
| **Command Handler** | 18–19 | `ICommandHandler<TCommand>` + pipeline behavior | `.../Handlers/` | `đang làm` | Dispatcher + **3/4 behavior**: Validation → Idempotency → Audit. Idempotency **giành chỗ trước** khi gọi handler (`Claim`/`Complete`/`Abandon`), nên thứ tự Validation → Idempotency giờ là correctness **thật** chứ không còn là preference. Lỗ cũ *"chặn lặp lại, không chặn đua"* **đã đóng một nửa**: đua trong một process hết (hoàn nguyên về check-then-act làm đỏ **6/292** test), đua **xuyên process** thì chưa — chỗ giữ mất theo process và hai instance không thấy nhau. **K7 vì thế CHƯA ĐẠT.** Cần cả **`TransactionBehavior`** lẫn một store bền vững, và cả hai cần database (M5, `ADR-023`) |
...
| **Multiplant Management** | 7. Multiplant Management | `SiteId` là first-class trong mọi entity, event, query, và trong Mendix XPath | `docs/scope.md` §5.6 | `đang làm` | `SiteId` bắt buộc trên `IDomainEvent`; là **đoạn đầu routing key** nên consumer bind được `nvm.NV1.#` và không bao giờ *nhận* message của site khác; seed có NV1 + DE1; **staged rollout giờ được chứng minh qua handler**, không còn là tuyên bố: NV1 chạy revision 3 trong khi DE1 còn ở 1, và event của NV1 không chứa path nào của DE1 (R3, `ADR-024`). Chưa có query/DB nào để ép filter phía server (M6) |
| **Factory Model** | 3. System Architecture | Cây ISA-95 `Enterprise/Site/Area/Line/WorkCell/Equipment` | `src/FunctionalBlocks/FactoryModel/` | `đang làm` | Cây 6 bậc thật cho NV1 + DE1, `EquipmentPath` tra O(1). **Ba revision đã publish** (`r1` 41 node · `r2` +4 kênh sạc · `r3` tháo `FORM-02`, thêm `STACK-04`), giữ trong `IFactoryModelCatalog` — revision là **tài liệu bất biến**, không sửa tại chỗ (`ADR-024`, R3). Chuyển revision 2 → 3 chạy qua handler và event báo đúng phần **thêm** lẫn phần **bớt**. Nguồn vẫn là **file seed**, chưa phải database (plan M1 §3.1), và revision đang hiệu lực vẫn mất khi restart — đó là lý do chưa `xong` |
