...
/// <summary>Which of the probe's consumers this run starts.</summary>
/// <remarks>
/// <para>
/// One switch rather than one flag per consumer, because the questions being asked need consumers
/// turned <b>off</b> as much as on. The claim that two queues are independent is only demonstrated by
/// stopping one consumer and watching its queue fill while the other keeps working — with both always
/// running, two log lines are equally explained by two queues and by one queue read twice.
/// </para>
/// <para>
/// The failing consumer is off by default for a different reason: left on, it would move every
/// message this worker sees into an error queue permanently, and an error queue that always has
/// messages in it is one nobody reads.
/// </para>
/// </remarks>
public static class ProbeSelection
{
    /// <summary>Environment variable holding a comma-separated list of roles.</summary>
    public const string Variable = "NVM_BUS_PROBE_CONSUMERS";

    /// <summary>The cache-updater probe.</summary>
    public const string Cache = "cache";

    /// <summary>The audit-trail probe.</summary>
