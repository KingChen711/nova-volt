...
| **App** | 10. App and Extension App Artifacts | Một host ASP.NET Core gom nhiều FB + expose Public Object Model | `src/Apps/Nvm.Host.All/` | `đang làm` | `Nvm.Host.All` đã gom FactoryModel, command pipeline, `Nvm.Time` và POM (`Nvm.PublicObjectModel`); `Nvm.App.Execution` chạy POM riêng trên Docker. Health live/ready đã chạy; readiness kiểm schema PostgreSQL. Chưa có FB thứ hai về command (C05/C06) |
| **Public Object Model** | 10 | **Chính là CQRS read side**: OData v4 endpoint trên read model Postgres | `src/Platform/Nvm.PublicObjectModel/` | `đang làm` | M4/C02: Equipment entity set có auth/site scope/paging/ETag, 21 integration test. Mendix connector PoC đã nghiệm thu. [ADR-013](adr/ADR-013-odata-cho-public-object-model.md). C03 thêm ProductionUnits/WipBoard |
| **Extension App** | 11, 21. Extension App Development | App Mendix mở rộng UI + gọi command | `mendix/`, `src/Apps/Nvm.App.Compliance/` | `đang làm` | App `NvmShopFloor` đăng nhập qua Keycloak, đọc OData POM có auth (M4/C02 PoC). Headers microflow truyền Bearer token, error handler xử lý 401/403. Chưa gọi command nào — ranh giới K10 chưa được đi qua lần nào |
| **UI Component** | 20. App Development Process | Mendix page/snippet dùng lại giữa nhiều app | `mendix/NvmShared/` | `đang làm` | Module `NvmShared` đã có: entity `NvmAccount`, Consumed OData Service `POM_v1`, external entity Equipment, 2 microflow headers/error handling. Chưa có snippet dùng lại |
