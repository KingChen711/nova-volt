...
| C08 · `feat(shopfloor): add the WIP board and recovery actions` · **runtime WIP/status đã kiểm** | Widget `mendix/widgets/wip-board` đọc external WipBoard qua SDK; 50nhóm/trang, auto-refresh5s sau completion, last-success/status cùng request, giữ snapshot khi lỗi. Wip_Board vẫn có nút mở draft. Model0errors | 2026-09-26: hai site đúng POM, slow7,5s không overlap; browser failure và backend stop/start giữ dữ liệu/time cũ rồi recovery;6unit tests. Còn gate input/p95 và workflow tổng thể; xem benchmarks/lab README |
