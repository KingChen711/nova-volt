...
| **Qualification / requalification** | Lần chứng minh một cấu hình production đạt ngưỡng đã chốt; **requalification** là chạy lại sau thay đổi có thể làm kết quả cũ hết giá trị. Ở NovaVolt, N1/N2 qualification diễn ra trước hoặc trong M9 và requalification ở M13 trên cùng rig đã qua preflight — không lấy một lần load dev làm chứng nhận vĩnh viễn |
...
| **Hợp đồng publish** *(publish contract)* | Cách một producer nói rằng file nó vừa ghi **đã xong**: ghi ra `<tên>.csv.partial`, đóng file, rồi **rename nguyên tử** thành `<tên>.csv.ready`. Chính phép rename đó **là** publish, và `*.csv.ready` là thứ duy nhất ingestion đọc. Cần có vì đổi tên một file **không** đóng handle mà exporter đang giữ — trên NFS nó ghi tiếp vào cùng inode sau khi ingestion đã claim, và ingestion lưu nửa run rồi xoá phần đuôi cùng claim, **không lỗi ở đâu cả**. Readiness nằm trong **chính tên file** chứ không ở một marker bên cạnh: hai file thì phải lấy bằng hai lần rename, và producer publish lại giữa hai lần đó làm người đọc lấy byte của export này dưới readiness của export khác (`ADR-035`). Hậu tố publish bắt buộc khác rỗng khi bật adapter; `.ready` là mặc định, không có fallback theo mtime (`ADR-036`). Ingestion trả file về inbox cũng publish đúng như vậy, nhưng luôn dưới một tên GUID mới — `mv` của producer đi xuyên qua mọi chỗ giữ chỗ, nên cách duy nhất để không xoá file của ai là không nhắm vào tên của họ |
...
| Từ | Nghĩa |
|---|---|
| **Historian** | Tầng lưu và truy vấn **telemetry quy trình theo thời gian**. Ở NovaVolt, TimescaleDB giữ điểm đo + rollup và MinIO giữ raw curve; historian không thay event store — nó trả lời *máy đã đo gì lúc nào*, còn event store trả lời *nghiệp vụ đã quyết định/chuyển trạng thái gì* |
...
| **Watermark** / **cửa sổ refresh** | Mốc mà continuous aggregate coi là "đã tính tới đây". `start_offset` và `end_offset` của policy định ra khoảng **policy thường xuyên nhìn lại**. Dữ liệu đến muộn hơn `start_offset` vẫn tạo invalidation, nhưng policy thường không refresh vùng cũ đó; rollup giữ số cũ cho tới một explicit wide refresh — và không có lỗi nào được ném (`ADR-032`) |
| **Real-time aggregation** | Chế độ mà truy vấn rollup tự nối thêm phần dữ liệu **mới hơn** watermark, tính trực tiếp từ bảng thô. Không sửa vùng cũ nằm dưới watermark. Ở đây đặt `materialized_only = true`: mọi query thấy đúng dữ liệu đã materialize, nên reconciliation nhìn được bucket nào đang cũ thay vì raw tail âm thầm che nó |
...
| **Compression policy** | Job nền tự nén chunk cũ hơn một tuổi nhất định (ở đây **7 ngày**). Chunk đã nén vẫn `SELECT` được và vẫn `INSERT` vào được; việc ghi muộn có thể đổi chi phí ghi nên phải đo trên workload thật, còn tính đúng đắn không được phép đổi |
| **Retention policy** | Job nền tự **drop chunk** cũ hơn một tuổi nhất định (thô **400 ngày**, rollup **15 năm**). Đây là một **cam kết pháp lý**, không phải tối ưu dung lượng: nó quyết định câu hỏi nào của auditor còn trả lời được. **Hai chân trời đó là chính sách; job thì hiện không chạy cái nào** — migration `007` gỡ job của raw và `010` gỡ job của rollup cho tới khi có **legal hold** (M12), vì `scope.md` §8.4 nói cờ đó chặn **mọi** retention policy, và nửa xoá không được chạy khi nửa chặn chưa tồn tại |
...
| **Object lock** *(MinIO/S3)* | Chế độ giữ bất biến **từng version** của object tới hết thời hạn. Với `COMPLIANCE`, version đã khoá không thể bị xoá hay rút ngắn retention; nhưng một `PUT` cùng key vẫn có thể tạo **version mới**, và xóa không chỉ rõ version vẫn có thể tạo delete marker. Vì vậy raw curve vừa lưu exact `version_id`, vừa dùng conditional write để không sinh bản thứ hai. Bucket `raw-curve` được tạo kèm lock ngay từ đầu và giữ 15 năm (`ADR-033`) |
...
| **Empty export** | File CSV có header đúng nhưng **không có dòng measurement**. Ở NovaVolt đây là lỗi vận hành phải vào `rejected`, không phải thành công 0 row: file không quy được về máy nào để archive, và có thể là dấu hiệu exporter đã bỏ mất cả run (`ADR-033`) |
